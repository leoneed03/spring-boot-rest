...
public interface UserPropertyRepo extends JpaRepository<UserProperty, Long> {
    ...
    @Query(value = "select * from TBL_PROP TP inner join TBL_USERS TU on TP.OWNER_USER_ID = TU.USER_ID where TU.USER_EMAIL = 'user_3@mail.ru'"
            , nativeQuery = true
    )
    List<UserProperty> getAllUserPropertyList();
